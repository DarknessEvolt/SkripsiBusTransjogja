...
/**
 * A route that consists of map nodes. There can be different kind of routes and
 * the type is determined by the type parameter ({@value #CIRCULAR} or
 * {@value #PINGPONG}).
 */
public class MapRoute {

    /**
     * Type of the route ID: circular ({@value}). After reaching the last node
     * on path, the next node is the first node
     */
    public static final int CIRCULAR = 1;
    /**
     * Type of the route ID: ping-pong ({@value}). After last node on path, the
     * direction on path is reversed
     */
    ...
    private static List<List<List<Coord>>> CoordsPerRoute  = new ArrayList<List<List<Coord>>>();;
    ...
    public static List<MapRoute> readRoutes(String fileName, int type,
        ...
        CoordsPerRoute.add(coords);
        return routes;
    }
    public static List<List<Coord>> getRouteBus (int index){
        return CoordsPerRoute.get(index);
    }
