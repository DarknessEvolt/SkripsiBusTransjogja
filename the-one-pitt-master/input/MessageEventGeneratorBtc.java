...
public class MessageEventGeneratorBtc implements EventQueue {
    ...
    public MessageEventGeneratorBtc(Settings s) {
        ...
        if (s.contains(TO_HOST_RANGE_S)) {
            this.toHostRange  = s.getCsvInts(HOST_RANGE_S, 2);
        }
     ...
     */
    protected int drawToAddress(int toHostRange[], int from) {
		 if (toHostRange[1] == toHostRange[0]) {
            return this.toHostRange[0];
        }
        return hostRange[0] + rng.nextInt(hostRange[1] - hostRange[0]);
	}
    /**
    ...
    public ExternalEvent nextEvent() {
        ...
        MessageCreateEventBtc mce;
        mce = new MessageCreateEventBtc(from, to, this.getID(),
